...
const outputRoot = join(rootPath, "generated", "copilot-studio");
...
function renderTopic(skill) {
  ...
  return `<!-- Generated by npm run compile. Do not edit directly. -->
# ${skill.title}
...
expectedFiles.set(
  join("generated", "copilot-studio", "README.md"),
...
for (const agent of catalog.agents) {
  ...
  const agentOutput = join("generated", "copilot-studio", agent.name);
...
if (checkOnly) {
  ...
  if (unexpectedFiles.length > 0 || staleFiles.length > 0) {
    ...
    console.log(`Generated Copilot Studio bundles are current (${catalog.agents.length} agents).`);
  }
} else {
  ...
  console.log(`Generated Copilot Studio bundles for ${catalog.agents.length} agents.`);
}