...
| `task-list-synchronizer` | Read-only; caller writes SharePoint list rows | Yes | No |
| `project-intake-triage` | Read-only; caller writes SharePoint list rows | Yes | No |
...
| `progress-collector` | Commit after preview, validation, and confirmation; caller writes SharePoint list rows | Yes | No |
| `cross-project-dependency-checker` | Read-only; caller writes SharePoint register and outcome rows | Yes | No |
| `deliverable-link-checker` | Read-only; caller writes SharePoint register and outcome rows | Yes | No |
...
5. For each generated topic file, create the corresponding Copilot Studio topic or prompt workflow and use its Markdown as the authoring specification. Reference configured MCP tools with the Copilot Studio slash menu where appropriate.
6. Require end-user confirmation for `commit_edit_draft` and any externally visible write.
7. Test the agent with the matching cases in `examples/evaluations.json` before publishing.
